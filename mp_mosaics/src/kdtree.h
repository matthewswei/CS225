...
class KDTree
{
  ...
  private:
     ...
     */
    void delete_(KDTreeNode* root);
    KDTreeNode* copy_(KDTreeNode* root, KDTreeNode* other);
    Point<Dim> find_helper(const Point<Dim>& target, KDTreeNode* node, int curDim) const;
    KDTreeNode* construct_help(int start, int end, int curDim);
    void quickselect(int start, int end, int k, int curDim);
    int partition(int start, int end, int index, int curDim);
};
