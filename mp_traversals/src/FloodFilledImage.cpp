...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  // Go through all the traversals in floodfill
  for (int i = 0; i<(int)traversals_.size(); i++) {
    // Get iterators for current traversal
    Traversals::ImageTraversal curr = traversals_[i];
    auto begin = curr.begin();
    auto end = curr.end();
    int count = 0;
    // Go through traversal and animate png
    for (auto it = begin; it!=end; ++it) {
      if (count%frameInterval==0) {
        animation.addFrame(png);
      }
      HSLAPixel& pixel = png.getPixel((*it).x, (*it).y);
      pixel = colors_[i]->getColor((*it).x, (*it).y);
      count++;
    }
  }
  animation.addFrame(png);
