                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size()==0 && mergeQueue.size()==0) {
        ...
        HuffmanTree::TreeNode* mergeNode =  mergeQueue.front();
        mergeQueue.pop();
        return mergeNode;
    } else if (mergeQueue.size()==0) {
        HuffmanTree::TreeNode* singleNode =  singleQueue.front();
        singleQueue.pop();
        return singleNode;
    }
    HuffmanTree::TreeNode* singleNode =  singleQueue.front();
    HuffmanTree::TreeNode* mergeNode =  mergeQueue.front();
    if (singleNode->freq<mergeNode->freq) {
        singleQueue.pop();
        return singleNode;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while ((singleQueue.size() + mergeQueue.size() !=1)) {
        ...
        TreeNode* node = new TreeNode(child1->freq.getFrequency() + child2->freq.getFrequency());
        node->left = child1;
        node->right = child2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        ...
        if (bit) {
            ...
            TreeNode* t = new TreeNode(Frequency(letter, 0));
            return t;
        } else {
            TreeNode* t = new TreeNode(Frequency('\0', 0));
